    ...
    public function bindParams($params)
{
    if (is_array($params) && !empty($params)) {
        ...
        foreach ($params as $key => $value) {
            $types .= 's'; // Assuming all parameters are strings; adjust as needed.
            $values[] = $value;
        }
        ...
        if ($stmt) {
            ...
            $this->error('Error preparing the query.');
        }
    }
...
public function update($table, $data)
{
    ...
    $updateData = array_merge($updateData, $this->array_wherein);
    ...
    if ($stmt) {
        // Bind parameters
        $types = str_repeat('s', count($updateData)); // Assuming all values are strings, adjust if needed
        $stmt->bind_param($types, ...$updateData);
    ...
    protected function _like($title, $match, $place, $type)
    {
        // If $title is an array, we need to process it

		$place = isset($place) ? $place : 'both';
        if (is_array($title)) {
...
public function get($table, $limit = null, $offset = null) {
    ...
    if ($offset) {
        if ($this->limit($limit, $offset)) {
            $stmt = $this->_mysqli->prepare("SELECT * FROM $table LIMIT ?, ?");
